...
async def do_login(page):
    try:
        ...
        # 步骤 C: [最终逻辑] 等待 "Next" 按钮可见并点击
        submit_btn_step1 = page.locator('button[type="submit"]')
        try:
            print_log("正在等待 'Next' (提交) 按钮变为[可见]...")
            await submit_btn_step1.wait_for(state="visible", timeout=30000)
        ...
        # 步骤 F: [最终逻辑] 等待 "Login" 按钮可见并点击
        submit_btn_step2 = page.locator('button[type="submit"]')
        try:
            print_log("正在等待 'Login' 按钮变为[可见]...")
            await submit_btn_step2.wait_for(state="visible", timeout=30000)
